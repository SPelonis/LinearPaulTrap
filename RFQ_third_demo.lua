...
adjustable T2_min = 26
adjustable T2_max = 42
...
for k = V_endcap_min, V_endcap_max, 5 do
...
function compute_KE(vx_v2, vy_v2, vz_v2)
  ...
  local KE = 1.05 * 0.5 * (average(vx_v2)^2 + average(vy_v2)^2 + average(vz_v2)^2)
...
local new_plane = TP(0,0,51.5,0,0,-1,
  function()
    -- mark()
    -- store variables for KE calculation
    local particle_count_v2 = #vx_v2 + 1
    vx_v2[particle_count_v2] = ion_vx_mm   -- store x-velocity (mm/usec)
    vy_v2[particle_count_v2] = ion_vy_mm   -- store y-velocity (mm/usec)
    vz_v2[particle_count_v2] = ion_vz_mm   -- store z-velocity (mm/usec)
...
function segment.fast_adjust()
  ...
  adj_elect01 = (tempvolts + 0.25*5.7)*0
  adj_elect02 = (- dcvolts + 0.25*5.7)*0  

  adj_elect03 = (tempvolts + 0.5*5.7)*0  
  adj_elect04 = (- dcvolts + 0.5*5.7)*0  

  adj_elect05 = (tempvolts + 0.75*5.7)*0  
  adj_elect06 = (- dcvolts + 0.75*5.7)*0  

  adj_elect07 = (tempvolts + 1.0*5.7)*0  
  adj_elect08 = (- dcvolts + 1.0*5.7)*0  

  adj_elect09 = tempvolts + 0.25*4.82*2  
  adj_elect10 = - dcvolts + 0.25*4.82*2  

  adj_elect11 = tempvolts + 0.50*4.82*2  
  adj_elect12 = - dcvolts + 0.50*4.82*2  

  adj_elect13 = tempvolts + 0.75*4.82*2  
  adj_elect14 = - dcvolts + 0.75*4.82*2  

  if (ion_time_of_flight <= i_clone) then
    adj_elect15 = tempvolts + 1.0*4.82*2  
    adj_elect16 = - dcvolts + 1.0*4.82*2  
  elseif (ion_time_of_flight > i_clone and ion_time_of_flight <= j_clone) then
    adj_elect15 = tempvolts + 1.0*4.82*2  
    adj_elect16 = - dcvolts + 1.0*4.82*2  
  elseif (ion_time_of_flight > j_clone) then
    adj_elect15 = tempvolts + V_add + 1.0*4.82*2  
    adj_elect16 = - dcvolts + V_add + 1.0*4.82*2
  end


  adj_elect17 = tempvolts + 1.0*4.82*2  
  adj_elect18 = - dcvolts + 1.0*4.82*2  

  if (ion_time_of_flight <= i_clone) then
    adj_elect19 = tempvolts + 1.0*4.82*2  
    adj_elect20 = - dcvolts + 1.0*4.82*2  
  elseif (ion_time_of_flight > i_clone and ion_time_of_flight <= j_clone) then
    adj_elect19 = tempvolts + k_clone/2 + 1.0*4.82*2  
    adj_elect20 = - dcvolts + k_clone/2 + 1.0*4.82*2   
  elseif (ion_time_of_flight > j_clone) then
    adj_elect19 = tempvolts + V_add + 1.0*4.82*2  
    adj_elect20 = - dcvolts + V_add + 1.0*4.82*2  
  end
